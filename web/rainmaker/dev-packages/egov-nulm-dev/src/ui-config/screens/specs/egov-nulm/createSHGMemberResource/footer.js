...
// import "./index.css";
  ...
  const moveToReview = dispatch => {
    const reviewUrl =`/egov-nulm/review-shg-member`;
    dispatch(setRoute(reviewUrl));
  };

export const callBackForNext = async (state, dispatch) => {
  let activeStep = get(
    state.screenConfiguration.screenConfig["create-shg-member"],
    "components.div.children.stepper.props.activeStep",
    0
  );
  const {NulmShgMemberRequest} = state.screenConfiguration.preparedFinalObject;
...
if(NulmShgMemberRequest ){
  if(!NulmShgMemberRequest.adharNo  && !NulmShgMemberRequest.adharAcknowledgementNo ){
    const errorMessage = {
...
if(NulmShgMemberRequest && ( !NulmShgMemberRequest.hasOwnProperty("gender") || !NulmShgMemberRequest.hasOwnProperty("caste"))){
  isFormValid = false;
}
  ...
  dispatch(
    prepareFinalObject("NulmShgMemberRequest.documentAttachemnt", documentAttachemnt)
  );
  dispatch(
...
export const changeStep = (
  ...
  dispatchMultipleFieldChangeAction("create-shg-member", actionDefination, dispatch);
