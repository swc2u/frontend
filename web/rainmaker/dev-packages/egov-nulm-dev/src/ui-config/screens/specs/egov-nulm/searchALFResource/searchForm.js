...
const resetFields = (state, dispatch) => {
  const textFields = ["applicationId","fromDate","toDate"];
  for (let i = 0; i < textFields.length; i++) {
    if (
      ...
      `state.screenConfiguration.screenConfig.search-smid-NGO.components.div.children.searchForm.children.cardContent.children.searchFormContainer.children.${textFields[i]}..props.value`
      ) {
      dispatch(
        handleField(
          "search-smid-NGO",
    ...
    applicationId: {
      ...getTextField({
        label: {
          ...
          labelKey: "NULM_ALF_APPLICATION_ID"
        },
        placeholder: {
          labelName: "Enter Application Id",
          labelKey: "NULM_ALF_APPLICATION_ID_PLACEHOLDER"
        },
