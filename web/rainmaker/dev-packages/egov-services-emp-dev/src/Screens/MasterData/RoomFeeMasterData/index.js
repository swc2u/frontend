...
class SimpleTable extends React.Component {
  async componentDidMount() {
    ...
    this.setState({ mdmsRes: Booking });
    console.log(this.state.mdmsRes, "stateresponse");
  ...
  async fetchTableData() {
    ...
    const foundUser =
      this.props.userInfo &&
      this.props.userInfo.roles.some((el) => el.code === "BK_ADMIN");
    if (foundUser) {
      let feeResponse = await httpRequest(
        "bookings/master/communitycenter/room/fee/_fetch",
        "_search",
        [],
        []
      );
  ...
  handleSearch(e) {
    ...
    filterfun.fn = (item) => {
      if (target.value === "") return item;
        ...
        return item.filter(
          (x) =>
            x.typeOfRoom.toLowerCase().includes(target.value.toLowerCase()) ||
            x.communityCenterName
              .toLowerCase()
              .includes(target.value.toLowerCase()) ||
            x.sectorName.toLowerCase().includes(target.value.toLowerCase()) ||
            x.rentFor3Hrs.toString().includes(target.value)||
            x.rentFor6Hrs.toString().includes(target.value)||
            x.rentFor9Hrs.toString().includes(target.value)||
            x.rentForOneDay.toString().includes(target.value)||
            x.totalNumberOfRooms.toString().includes(target.value)
        );
      }
    };
    ...
    return this.state.isLoading === true ? (
      ...
      <div>
        ...
        <div class="col-xs-12 " style={{ wordBreak: "break-word" }}>
          <Paper style={{ overflowX: "auto" }}>
            <div>
              ...
              <TableUi
                ...
                columnHideIndex={this.state.columnHideIndex}
                filterfun={this.state.filterfun}
          ...
          <div>
            <DialogComponent
              mdmsRes={this.state.mdmsRes}
              validFromDate={this.state.validFromDate}
              open={this.state.open}
              handleClose={this.handleClose.bind(this)}
              updateMasterData={this.state.updateData}
            />
          </div>
        </div>
      </div>
...
export default withStyles(styles)(connect(mapStateToProps, null)(SimpleTable));
