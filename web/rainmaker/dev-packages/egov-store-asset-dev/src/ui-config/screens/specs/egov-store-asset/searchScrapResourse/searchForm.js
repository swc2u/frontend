...
const resetFields = (state, dispatch) => {
  const textFields = ["store","scrapNumber"];
  for (let i = 0; i < textFields.length; i++) {
    if (
      `state.screenConfiguration.screenConfig.search-scrap-material.searchForm.children.cardContent.children.searchFormContainer.children.${textFields[i]}.props.value`
    ) {
      dispatch(
        handleField(
...
export const searchForm = getCommonCard({
  ...
  searchFormContainer: getCommonContainer({
    scrapNumber: getTextField({
      ...
      placeholder: {
        labelName: "Enter Scrap Number",
        labelKey: `STORE_SCRAP_NUMBER_PLACEHLDR`,
      },
     // pattern: getPattern("alpha-only"),
      errorMessage: "ERR_DEFAULT_INPUT_FIELD_MSG",

      jsonPath: "searchScreen.scrapNumber",
    }),
