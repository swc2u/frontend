  ...
  if (!isSearchFormValid) {
    ...
    for (var key in searchScreenObject) {
      if (
        ...
        if (key === "issueDate" || key === "issueToDate"|| key === "issueFromDate") {
         let Dateselect = true;
          queryObject.push({
            key: key,
            value: key === 'issueT0Date'?  convertDateToEpoch(searchScreenObject[key]) : convertDateToEpoch(searchScreenObject[key], "daymid")
          });
        } 
        else
    ...
    try {
      ...
      let data = response.materialIssues.map((item) => {
        ...
        return {
         ...
         [getTextToLocalMapping("Indenting Store Name")]: get(item, "toStore.name", "-") || "-", 
