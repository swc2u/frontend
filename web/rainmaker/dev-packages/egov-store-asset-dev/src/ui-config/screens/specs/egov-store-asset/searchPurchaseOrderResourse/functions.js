...
export const searchApiCall = async (state, dispatch) => {
  ...
  if (!isSearchFormValid) {
    ...
    // Add selected search fields to queryobject , "daymid"
    for (var key in searchScreenObject) {
      ...
      if(searchScreenObject.hasOwnProperty(key) && typeof searchScreenObject[key] === "boolean"){
        ...
        let Dateselect = true;
         queryObject.push({
           key: key,
           value: key === "indentFromDate"? convertDateToEpoch(searchScreenObject[key], "daymid"):convertDateToEpoch(searchScreenObject[key])
         });
       } 
      else  if (
