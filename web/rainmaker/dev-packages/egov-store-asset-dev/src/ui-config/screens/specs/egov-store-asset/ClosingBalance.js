...
import {  
  ...
  const resetFields = (state, dispatch) => {
    ...
    for (let i = 0; i < textFields.length; i++) {
      if (
        `state.screenConfiguration.screenConfig.ClosingBalance.components.div.children.SearchCard.children.cardContent.children.appPRSearchContainer.children.${textFields[i]}.props.value`
      ) {
        dispatch(
          handleField(
            "ClosingBalance",
...
{
  const textFields = ["storeName","asOnDate",];
  for (let i = 0; i < textFields.length; i++) {
    if (
      `state.screenConfiguration.screenConfig.ClosingBalance.components.div.children.SearchCard.children.cardContent.children.appPRSearchContainer.children.${textFields[i]}.props.value`
    ) {
  ...
  if(!isProfessionalDetailsValid)
  {
    ...
    try {
     ...
     let Responce = await httpRequest(
        "post",
        "store-asset-services/openingbalance/_closingReport",
        "_report",    
        queryObject
      );
...
const header = getCommonHeader({
  labelName: "Closing Balence Report",
  labelKey: "STORE_CB_REPORT"
});
