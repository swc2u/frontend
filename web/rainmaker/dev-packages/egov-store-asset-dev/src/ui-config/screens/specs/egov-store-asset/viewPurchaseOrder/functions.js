  ...
  if (action === "CREATE") {
    try {
      ...
      if (response) {
        ...
        dispatch(setRoute(`/egov-store-asset/view-purchase-order?applicationNumber=${response.purchaseOrders[0].purchaseOrderNumber}&tenantId=${response.purchaseOrders[0].tenantId}`));
