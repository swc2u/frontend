...
class WorkFlowContainer extends React.Component {
  ...
  getRedirectUrl = (action, businessId, moduleName) => {
    ...
    if (dataPath === "indents") {
      ...
      let issueIndentNumber = indentissuedata.indent.indentNumber
      let response = preparedFinalObject.indents.filter(x => x.indentNumber === issueIndentNumber)
      if (response && response[0])
        editUrl = `/egov-store-asset/createMaterialIndentNote?step=0&IndentId=${response[0].id}`;
    }
